...
class LineRecordScreen extends StatefulWidget {
  final String projectName;

  const LineRecordScreen({required this.projectName, super.key});

  @override
  State<LineRecordScreen> createState() => _LineRecordScreenState();
}

class _LineRecordScreenState extends State<LineRecordScreen> {
  ...
  Future<void> _startListening() async {
    ...
    _subscription = Geolocator.getPositionStream(
      ...
      (position) {
        if (!mounted) return;
        setState(() {
          final added = _recorder.add(position);
          if (added) _restartTicker();
        });
      },
      onError: (Object _) {
        if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    if (vertices.length < 2 || _saving) return;
    ...
    try {
      ...
      final responses = jsonEncode({
        ...
        'distance_m': _recorder.totalDistanceM,
      ...
      await _database.insertSurveySession(
        SurveySessionsCompanion(
          ...
          title: drift.Value(
            'Line • ${GeometryService.formatDistance(_recorder.totalDistanceM)}'
            ' • ${vertices.length} pts',
          ),
      ...
      _showSnack('Line saved');
    } catch (e) {
      _showSnack('Failed to save line: $e');
    } finally {
      if (mounted) {
        setState(() {
          _saving = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Record Line')),
      body: Padding(
        ...
        children: [
          Row(
            ...
            children: [
              _StatItem(
                label: 'Distance',
                value: GeometryService.formatDistance(_recorder.totalDistanceM),
              ),
  ...
  Widget _buildControls() {
    ...
    return Column(
      ...
      children: [
        ...
        FilledButton.icon(
          onPressed:
              _recorder.vertices.length >= 2 && !_saving ? _save : null,
          icon: _saving
