...
## Part I — Cloud Sync (Tasks 1-5) — v2.2.0

| # | Step | Command | Result |
|---|------|---------|--------|
| I1 | schema (Task 1) | `lib/data/app_database.dart` edit + `dart run build_runner build --delete-conflicting-outputs` | Added `synced_at`/`photo_synced_at` nullable to `survey_sessions`, new `sync_configs` table (`projectId` PK CASCADE, `syncEndpointUrl`/`syncApiKey`/`lastSyncAt` nullable), bump 8→9, migration `from<9` add columns + `createTable`, `deleteProject`/`resetAllData` cleanup, helper `getSyncConfig`/`upsertSyncConfig`/`deleteSyncConfig` + `testWithExecutor` helper; codegen 32 s 198 outputs |
| I2 | migration test | `flutter test test/sync_schema_migration_test.dart` | 4 tests: fresh schema nullable checks, insert defaults null + update round-trip, `sync_configs` CRUD + `ON DELETE CASCADE` DDL, v8→v9 upgrade simulation via raw `sqlite3` old DB (`user_version=8`) → `NativeDatabase(file)` migration asserts old row `syncedAt` null + new cols/table created |
| I3 | settings UI (Task 2) | `lib/ui/project_detail_screen.dart` edit | Cloud Sync section (URL `TextInputType.url` + API key obscured `•`+visibility toggle, `lastSyncAt` line, Save → `upsertSyncConfig` green *Sync settings saved*, Test Connection → `http.get` 10 s, validates `http/https` scheme, checks `200-299` + JSON `ok==true` specifically → green *Connection successful* else red *missing {ok:true}*/`not valid JSON`/`HTTP xxx`/`Invalid URL`; empty → *Please enter…*; no crash on malformed |
| I4 | data sync (Task 3) | `lib/services/cloud_sync_service.dart` + `test/cloud_sync_service_test.dart` | `queryUnsyncedResponses`/`Features` where `syncedAt IS NULL` + `!draft` + `feature_type` split; payload `{"apiKey","action":"sync_data","responses":[{"response_id"=id, "project_name","surveyor","submitted_at","form_name","answers"}],"features":[{"feature_id","project_name","surveyor","geometry_type","latitude","longitude","geojson","photo_path"}]}` (`id.toString()` stable), POST JSON 30 s single attempt, on `{ok:true}` transaction marks `syncedAt`/`lastSyncAt`, else surfaces `error` field; tests via `HttpServer` mock: success marks rows + payload shape, fail leaves unsynced, `Invalid API key` surfaced, 500/generic cases |
| I5 | photo sync (Task 4) | `lib/services/photo_sync_service.dart` + `lib/services/sync_orchestrator.dart` + `test/photo_sync_service_test.dart` | Per-photo `{"apiKey","action":"upload_photo","filename","mimeType","base64"}` individually, 3-attempt retry backoffs 2 s/5 s (`_delay` injectable), only `photo_synced_at` on `{ok:true}`, continue after 3 fails (one bad never blocks), `>15 MB` skipped with `skippedOversized` no network; `PhotoSyncResult` + `SyncOrchestrator.syncAll` data→photos sequence with `FullSyncResult.summary` *“X responses, Y features, A/B photos synced”*; tests: 2 fails→success retry (3 attempts), one always-fail blocked skips without blocking next (4 requests), oversized threshold 100 B → 0 requests, query filter |
| I6 | home sync (Task 5) | `lib/ui/home_screen.dart` edit | Added imports `cloud_sync_service`/`photo_sync_service`/`sync_orchestrator`/`project_detail_screen`; `_handleSync` checks selected project + `getSyncConfig` → if no URL → SnackBar + push `ProjectDetailScreen`, else `showDialog` `_SyncProgressDialog`; `_isOfflineError` maps `SocketException`/`failed host lookup`/`network`/`timed out` → *No internet connection*; `_formatLastSynced` + `_buildSyncCard` (FutureBuilder keyed by `_refreshTick`, `Set up cloud sync` grey vs `Sync` purple `cloud_sync` card, `Last synced: YYYY-MM-DD HH:MM`/`Never synced`) placed after GPS Mode before Collected Data; `_SyncProgressDialog` stateful: `Syncing data…` → `Syncing photos (3/9)…` live via `onProgress`, then `Sync complete` summary or red *Sync failed*; added `onProgress` to `PhotoSyncService.syncPhotos`; fixed `cloud_sync_service` wildcard lint, `photo_sync_service` `!` promotion, patched `project_screens_test` scrollable to vertical |
